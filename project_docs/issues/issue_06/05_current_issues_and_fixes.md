...
Custom AutoAPI templates exist in PyDevelop-Docs `/docs/_autoapi_templates/` but are never distributed to projects that use `pydevelop-docs init`. This means all the enhanced templates we've created (Pydantic model detection, better class organization, etc.) are not available to end users.
...
3. Test with real projects:
   - Generate new project with pydevelop-docs init
...
1. Generate test project: pydevelop-docs init test-enhanced-templates
