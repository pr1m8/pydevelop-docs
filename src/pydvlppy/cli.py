"""Command-line interface for Pydvlppy.

This module provides the main CLI commands for Pydvlppy, enabling users to
...
Examples:
    ...
    >>> pydvlppy interactive

    Analyze any Python project:

    >>> pydvlppy analyze /path/to/project

    Universal setup for any project:

    >>> pydvlppy setup-general /path/to/project --force

    Copy setup between projects:

    >>> pydvlppy copy-setup /source/project /dest/project
...
class ProjectAnalyzer:
    ...
    def _analyze_dependencies(self) -> dict[str, Any]:
        ...
        try:
            ...
            # pydvlppy is a documentation tool, not a dependency manager
...
# -- Import shared configuration from pydvlppy -------------------------
...
def build_all(clean, ignore_warnings):
    ...
    if not (project_path / "packages").exists():
        ...
        click.echo("   Use 'pydvlppy build' for single packages.")
        raise click.Abort()
...
def link_docs(clean, open, update_only):
    ...
    for cross-referencing. Uses the complete pydvlppy configuration
...
def rebuild_haive(packages, no_master, no_clean, quiet, debug, save_log):
    ...
    Examples:
        # Rebuild everything
        pydvlppy rebuild-haive

        # Rebuild specific packages only
        pydvlppy rebuild-haive -p haive-core -p haive-agents

        # Rebuild with detailed logging
        pydvlppy rebuild-haive --debug --save-log

        # Quick rebuild without master hub
        pydvlppy rebuild-haive --no-master
    """
...
def setup():
    ...
    click.echo("   4. Run 'pydvlppy watch' for auto-rebuild")
...
def setup_general(project_path, target_dir, force, non_interactive, dry_run, copy_to):
    ...
    Examples:
        ...
        pydvlppy setup-general

        # Setup docs for specific project
        pydvlppy setup-general /path/to/my-project

        # Dry run to see what would be done
        pydvlppy setup-general --dry-run

        # Copy setup to another location
        pydvlppy setup-general --copy-to /path/to/copy/destination
    """
...
@click.option("--include-config", "-c", is_flag=True, help="Include Pydvlppy configuration files")
...
def copy_setup(source_path, destination_path, include_config, include_static, force):
    ...
    Examples:
        ...
        pydvlppy copy-setup /path/to/source /path/to/destination

        # Copy with configuration files
        pydvlppy copy-setup /path/to/source /path/to/dest --include-config

        # Copy everything including static assets
        pydvlppy copy-setup /path/to/source /path/to/dest --include-config --include-static
    """
    import shutil
...
def test(coverage, verbose, fast, integration, unit, lint, format_check, type_check, run_all):
    """Run tests and code quality checks for Pydvlppy.
    ...
    click.echo("🧪 Running Pydvlppy tests...")
...
try:
    ...
    if result.returncode == 0 and 'pydvlppy' in result.stdout:
...
def publish(test, dry_run, force):
    ...
    try:
        ...
        if test:
            ...
            click.echo("📝 Installation: pip install pydvlppy")
...
def publish_docs(check_links, upload_rtd, deploy_pages):
    ...
    """
    click.echo("📚 Publishing Pydvlppy documentation...")
    ...
    if upload_rtd:
        ...
        click.echo("   🔗 https://readthedocs.org/projects/pydvlppy/")
...
def release(check_version, dry_run, part):
    ...
    """
    click.echo("🎯 Starting Pydvlppy release workflow...")
