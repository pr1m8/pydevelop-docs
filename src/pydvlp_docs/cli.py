...
class ProjectAnalyzer:
    ...
    def _analyze_dependencies(self) -> dict[str, Any]:
        ...
        try:
            ...
            # pydvlp-docs is a documentation tool, not a dependency manager
...
# -- Import shared configuration from pydvlp_docs -------------------------
...
def build_all(clean, ignore_warnings):
    ...
    if not (project_path / "packages").exists():
        ...
        click.echo("   Use 'pydvlp-docs build' for single packages.")
        raise click.Abort()
...
def link_docs(clean, open, update_only):
    ...
    for cross-referencing. Uses the complete pydvlp-docs configuration
...
def rebuild_haive(packages, no_master, no_clean, quiet, debug, save_log):
    ...
    Examples:
        # Rebuild everything
        pydvlp-docs rebuild-haive

        # Rebuild specific packages only
        pydvlp-docs rebuild-haive -p haive-core -p haive-agents

        # Rebuild with detailed logging
        pydvlp-docs rebuild-haive --debug --save-log

        # Quick rebuild without master hub
        pydvlp-docs rebuild-haive --no-master
    """
...
def setup():
    ...
    click.echo("   4. Run 'pydvlp-docs watch' for auto-rebuild")
...
try:
    ...
    if result.returncode == 0 and 'pydvlp-docs' in result.stdout:
