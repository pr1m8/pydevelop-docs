...
class HaiveDocumentationManager:
            ...
            if result.returncode == 0:
                ...
                if index_file.exists():
                    self.log_operation(
                        "build_package",
                        "success",
                        f"Built {package_name} ({index_file.stat().st_size // 1024}KB)",
                        duration,
                    )
                ...
                self.log_operation(
                    "build_package",
                    "error",
                    f"Failed to build {package_name}: {error_msg}",
                    duration,
                )
    ...
    def build_master_docs(self, clean: bool = True) -> bool:
        ...
        try:
            # Use pydevelop-docs link-docs command for master hub
            cmd = ["poetry", "run", "pydevelop-docs", "link-docs"]
